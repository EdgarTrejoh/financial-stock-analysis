#1. Import libraries and configure the home page
...
#@st.cache_data
def load_income_statement(): #historical data
    data_is = conn.read(worksheet="Income_Statement", usecols=list(range(14)), ttl=2)
    data_is = data_is[data_is["Empresa"]==st.session_state["selected_company"]]
    data_is = data_is.dropna(how="all")
    return data_is

#@st.cache_data
def load_current_income_statement(): #current data
    data_current_is = conn.read(worksheet="Current_IS", usecols=list(range(14)), ttl=2)
    data_current_is = data_current_is[data_current_is["Empresa"]== st.session_state["selected_company"]]
    data_current_is = data_current_is.dropna(how="all")
    return data_current_is

#@st.cache_data
def load_balance_sheet(): #historical data
    data_bs = conn.read(worksheet="Balance_Sheet", usecols=list(range(38)), ttl=2)
    data_bs = data_bs[data_bs["Empresa"]==st.session_state["selected_company"]]
    data_bs = data_bs.dropna(how="all")
    return data_bs

#@st.cache_data
def load_current_balance(): #current data
    data_current_bs = conn.read(worksheet="Current_BS", usecols=list(range(38)), ttl=2)
    data_current_bs = data_current_bs[data_current_bs["Empresa"]==st.session_state["selected_company"]]
    data_current_bs = data_current_bs.dropna(how="all")
    return data_current_bs

data_load_state = st.markdown(":red[Loading data...]")
income_statement = load_income_statement()
income_statement_current = load_current_income_statement()
balance_sheet = load_balance_sheet()
balance_sheet_current = load_current_balance()
data_load_state.markdown(':blue[Loading data... done!]')

income_statement['Periodo'] = income_statement['Periodo'].astype(str)
income_statement_current['Periodo'] = income_statement_current['Periodo'].astype(str)
...
Profit_margin_current = pd.DataFrame({
    'Period': income_statement_current['Periodo'],
...
Financial_leverage_current = pd.DataFrame({
    'Period': income_statement_current['Periodo'],
    "Financial Leverage": (balance_sheet_current['Totalassets']/balance_sheet_current['Totalstockholders’equity']) 
})
...
ROE_components_current = pd.DataFrame({
    'Period': Financial_leverage_current['Period'],
...
Debt_to_assets_ratio_current = (balance_sheet_current['Totalliabilities'].iloc[0])/(balance_sheet_current['Totalassets'].iloc[0])
Debt_to_equity_ratio_current = (balance_sheet_current['Totalliabilities'].iloc[0])/(balance_sheet_current['Totalstockholders’equity'].iloc[0])

# Liquidity Ratios
current_ratio = balance_sheet_current['Totalcurrentassets'].div(balance_sheet_current['Totalcurrentliabilities'])
acid_test = (balance_sheet_current['Totalcurrentassets'] -balance_sheet_current['Inventory']).div(balance_sheet_current['Totalcurrentliabilities'])
...
revenues_chart.update_layout(
        title_text=f"{empresa} - Revenues - Cost of Revenues",
        title_font=dict(
            color="#027034",
            size=20
            ),
...
current_revenue = income_statement_current['Revenues']
...
with current_year:
    ...
    v1.write("")
    m1.metric(":blue[Revenues]",  millify(income_statement_current['Revenues'].iloc[0]))
    m2.metric(':blue[Total Costs]', millify(total_costs))
    m3.metric(':blue[Net Income]', millify(income_statement_current['Net income'].iloc[0]))
    v2.write("")
