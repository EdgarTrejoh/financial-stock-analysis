...
dataframe = conn.read(worksheet="Tickers", usecols=[1,2], ttl=2)
dataframe = dataframe.dropna(how="all")
stocks = dataframe['Company']
stocks_sorted = sorted(stocks)

st.markdown("# :green[1. Technical Analysis 📈]")

empresa =st.selectbox(":blue[***Please select an option:***]", stocks_sorted, help = 'Filter report to show only one Stock') 

# Find the corresponding row in the DataFrame
selected_company = dataframe[dataframe['Company']==empresa]

#Get the value of the "Ticker" colmun for selected company
ticker = selected_company.iloc[0]["Ticker"]
